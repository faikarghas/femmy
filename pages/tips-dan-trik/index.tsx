    ...
    <Layout page="tentang-kami">
      <section className="relative bg-femmy-plight py-8 px-8 lg:px-16">
        ...
        {tips.map((val,i)=>{
            if (i === 0) {
              return (
                <div key={i} className="grid grid-cols-1 lg:grid-cols-3 mb-8 gap-0 lg:gap-10 bg-white lg:bg-transparent rounded-tr-2xl rounded-tl-2xl">
                  <div className="lg:col-span-1 lg:rounded-2xl">
                    <img src={`/images/news/${val.image}`} className="rounded-tr-2xl rounded-tl-2xl lg:rounded-2xl" />
                  </div>
