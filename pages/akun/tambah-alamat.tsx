...
const TambahAlamat: NextPage = ({auth,user,provinsi,kabupaten,kecamatan}:any) => {
                        ...
                        <div>
                            <div className='flex flex-wrap'>
                                <div className='py-8 px-16 basis-full h-[516px] overflow-y-scroll'>
                                   <h3 className='text-[20px] font-sansSemi text-femmy-pdark mb-8'>Tambah Alamat</h3>
...
export const getServerSideProps =  wrapper.getServerSideProps( store => async ({req, res}:any) => {
    ...
    if(token){
        ...
        const fetchKecamatan = await fetch(`${process.env.NEXT_PUBLIC_API}/api/kecamatans`,{
            method:"GET",
            headers:{
                'Authorization': 'Bearer ' + req.cookies.refreshToken,
            }
        })
        kecamatan = await fetchKecamatan.json()
